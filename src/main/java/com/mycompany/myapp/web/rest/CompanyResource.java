...
public class CompanyResource {
    ...
    @RequestMapping(value = "/companies/myTesting",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public  ResponseEntity myTesting() throws Exception
    {
        companyService.myLogic("Hello World");

        return new ResponseEntity("ok", HttpStatus.OK);
    }

    @RequestMapping(value = "/companies/getAll",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public  ResponseEntity check() throws Exception
    {
        String msg = companyService.myLogic("hallo");

        ResponseEntity httpRes = null;

        Iterable dataRepo = companyRepository.findAll();

        Iterable dataRepoByName = companyRepository.findByName("dea");


        Map<String, Object> response = new HashMap<>();

        response.put("status", "OK");
        response.put("arrayData", dataRepo);

        httpRes = new ResponseEntity(response, HttpStatus.OK);
        return httpRes;
    }
